...
export const paginationRender = (): Node => {
    ...
    prevButton.addEventListener("click", async () => {
        currentPage > 1 && (currentPage--, await updatePagination(currentPage));
    });
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        i === 1 && paginationButton.classList.add("active");
    ...
    nextButton.addEventListener("click", async () => {
        (currentPage < totalPages) && (currentPage++, await updatePagination(currentPage));
    });
...
const updatePagination = async (currentPage: number) => {
    ...
    buttons.forEach((button) => {
        button.classList[parseInt(button.getAttribute("data-page") || "0")
         === currentPage ? "add" : "remove"]("active");
    });
};
